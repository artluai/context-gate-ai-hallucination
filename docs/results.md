...
| DeepSeek V4 Flash | Token gate | 0.3% | 1.2% | One tiny exact-name typo |
...
| Claude Opus 4.7 Medium | Token gate | 15.0% | 35.7% | 3 malformed JSON runs counted as drift |
...
| DeepSeek V4 Flash | Rule pasted in | 0.0% | 0.0% | Passed 20/20 |
| DeepSeek V4 Flash | Token gate | 0.0% | 0.0% | Passed 20/20 |
...
| Claude Opus 4.7 Medium | Token gate | 10.8% | 29.8% | 2 malformed JSON runs counted as drift |
...
| DeepSeek V4 Flash | Token gate | 0.0% | 0.0% |

## Interpretation

The main pattern is not that token gates make models perfect. The main pattern
...
Force-feeding the source of truth sharply reduces drift. The token adds
enforcement and auditability: a downstream step can refuse to continue unless
the gate was passed.
...
> Rule-gating reduces one common cause of hallucination: answering before the
